...
object PrivacyLogScrubber {
    ...
    private val MNEMONIC_PATTERN = Pattern.compile("(?i)(?:[a-z]{3,8}\\s+){11,23}[a-z]{3,8}")
    ...
    fun scrub(message: String): String {
        ...
        // 1. Scrub Mnemonics
        scrubbed = MNEMONIC_PATTERN.matcher(scrubbed).replaceAll(REDACTED)

        // 2. Scrub Key Material (Hex / Base64)
        scrubbed = KEY_MATERIAL_PATTERN.matcher(scrubbed).replaceAll(REDACTED)

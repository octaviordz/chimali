...
class GetUserConsentUseCaseTest {
    ...
    fun setUp() = runTest {
        ...
        testConsentRecord = UserConsentRecord.create(
    ...
    inner class SuccessfulConsentRecordingTests {
        ...
        fun `should successfully record consent without credential id`() = runTest {
            val result = getUserConsentUseCase(
                rpId = testRpId,
                operationType = ConsentOperationType.REGISTRATION,
                credentialId = null,
        ...
        fun `should use custom prompt when provided`() = runTest {
    ...
    inner class ConsentRetrievalTests {
        ...
        fun `should retrieve consent records by operation type`() = runTest {
            val registrationConsent = UserConsentRecord.create(
                operationType = ConsentOperationType.REGISTRATION,
                rpId = testRpId,
                biometricUsed = true,
                pinUsed = false
            )
            val authConsent = UserConsentRecord.create(
                operationType = ConsentOperationType.AUTHENTICATION,
                rpId = testRpId,
                biometricUsed = false,
                pinUsed = true
            ...
            val result = getUserConsentUseCase.getConsentRecordsByOperationType(
        ...
        fun `should retrieve consent records by credential id`() = runTest {
            val targetCredentialId = "target_credential_id"
            val targetConsent = UserConsentRecord.create(
                operationType = ConsentOperationType.REGISTRATION,
                rpId = testRpId,
            ...
            val otherConsent = UserConsentRecord.create(
                operationType = ConsentOperationType.AUTHENTICATION,
                rpId = testRpId,
        ...
        fun `should retrieve consent records by rp id`() = runTest {
        ...
        fun `should retrieve consent records by time range`() = runTest {
            val startTime = Instant.now().minusSeconds(3600) // 1 hour ago
            val endTime = Instant.now().plusSeconds(3600) // 1 hour from now
            
            val inRangeConsent = UserConsentRecord.create(
                operationType = ConsentOperationType.REGISTRATION,
                rpId = testRpId,
                biometricUsed = true,
                pinUsed = false
            )
            val outOfRangeConsent = UserConsentRecord.create(
                operationType = ConsentOperationType.AUTHENTICATION,
                rpId = testRpId,
                biometricUsed = false,
                pinUsed = true
    ...
    inner class ConsentStatisticsTests {
        ...
        fun `should calculate consent statistics correctly`() = runTest {
            val consentRecords = listOf(
                UserConsentRecord.create(
                    operationType = ConsentOperationType.REGISTRATION,
                    rpId = testRpId,
        ...
        fun `should calculate statistics for specific rp`() = runTest {
            val targetRpConsent = UserConsentRecord.create(
                operationType = ConsentOperationType.REGISTRATION,
                rpId = testRpId,
                biometricUsed = true,
                pinUsed = false
            )
            val otherRpConsent = UserConsentRecord.create(
                operationType = ConsentOperationType.AUTHENTICATION,
                rpId = "https://other.com",
                biometricUsed = false,
                pinUsed = true
        ...
        fun `should identify most used consent method`() = runTest {
            val consentRecords = listOf(
                UserConsentRecord.create(
                    operationType = ConsentOperationType.REGISTRATION,
                    rpId = testRpId,
    ...
    inner class RecentConsentCheckTests {
        ...
        fun `should correctly identify recent consent`() = runTest {
            val recentConsent = UserConsentRecord.create(
                operationType = ConsentOperationType.REGISTRATION,
                rpId = testRpId,
                biometricUsed = true,
                pinUsed = false
            ...
            val result = getUserConsentUseCase.isRecentConsentGranted(
                rpId = testRpId,
                operationType = ConsentOperationType.REGISTRATION,
                minutes = 5
            )
        ...
        fun `should correctly identify non recent consent`() = runTest {
            val oldConsent = UserConsentRecord.create(
                operationType = ConsentOperationType.REGISTRATION,
                rpId = testRpId,
                biometricUsed = true,
                pinUsed = false
        ...
        fun `should handle no consent records`() = runTest {
    ...
    inner class ValidationFailureTests {
        ...
        fun `should fail when rp id is blank`() = runTest {
            val result = getUserConsentUseCase(
                rpId = "",
                operationType = ConsentOperationType.REGISTRATION,
                credentialId = "test_credential_id",
        ...
        fun `should fail when rp id is invalid`() = runTest {
            val result = getUserConsentUseCase(
                rpId = "invalid-rp-id",
                operationType = ConsentOperationType.REGISTRATION,
                credentialId = "test_credential_id",
        ...
        fun `should fail when credential id is blank`() = runTest {
            val result = getUserConsentUseCase(
                rpId = testRpId,
                operationType = ConsentOperationType.REGISTRATION,
                credentialId = "",
        ...
        fun `should fail when credential id exceeds maximum length`() = runTest {
    ...
    inner class OperationTypeTests {
        ...
        fun `should use appropriate prompts for different operation types`() = runTest {
            val operationTypes = listOf(
    ...
    inner class EdgeCases {
        ...
        fun `should handle maximum allowed credential id length`() = runTest {
        ...
        fun `should handle empty consent statistics`() = runTest {
            coEvery { credentialRepository.getRecentUserConsent(any(), any()) } returns flowOf()

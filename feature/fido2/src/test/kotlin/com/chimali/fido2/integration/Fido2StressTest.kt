...
class Fido2StressTest {
    ...
    @Test
    fun `100 consecutive authentication operations on registered credentials succeed at a 95 percent rate`() = runTest {
    ...
    private fun buildMakeCredentialOptions(userId: String, rpIdHost: String): MakeCredentialOptions {
...
private class InMemoryCredentialRepository : CredentialRepository {
    ...
    override suspend fun getAllCredentials(): Flow<PasskeyCredential> =
        flowOf(*credentials.values.toTypedArray())
    ...
    override suspend fun credentialExists(rpId: String, userId: String): Boolean =
        credentials.values.any { it.rpId == rpId && it.userId == userId }
    ...
    override suspend fun getCredentialCountByRpId(rpId: String): Int =
        credentials.values.count { it.rpId == rpId }
    ...
    override suspend fun getCredentialStatistics(): CredentialStatistics = CredentialStatistics(
    ...
    override suspend fun deleteAllCredentials(rpId: String?): Result<Unit> {
        if (rpId == null) {
            ...
            toRemove.forEach { credentials.remove(it); signCounts.remove(it) }
        }
        return Result.success(Unit)
    }

...
class Ctap2CredentialManagementHandlerTest {
    ...
    fun setup() {
        ...
        handler = Ctap2CredentialManagementHandler(
            cborCodec = cborCodec,
    ...
    @Test
    fun `handle returns CTAP1_ERR_MISSING_PARAMETER if subCommand is missing`() = runTest {
    ...
    @Test
    fun `handle returns CTAP2_ERR_UNSUPPORTED_OPTION for unknown subCommand`() = runTest {
    ...
    @Test
    fun `handleGetCredsMetadata returns correct credential count`() = runTest {
    ...
    @Test
    fun `enumerateRPsBegin returns first RP with totalRPs count`() = runTest {
    ...
    @Test
    fun `enumerateRPsBegin returns NO_CREDENTIALS when empty`() = runTest {
    ...
    @Test
    fun `enumerateRPsGetNextRP returns next RP after Begin`() = runTest {
    ...
    @Test
    fun `enumerateRPsGetNextRP returns NOT_ALLOWED when exhausted`() = runTest {
    ...
    @Test
    fun `enumerateCredentialsBegin returns first credential with totalCredentials`() = runTest {
        val cred1 = createTestCredential("cred1", "https://example.com", "user1")
        ...
        every { cborCodec.decodeFromFido2Format(any()) } returns mapOf(
            "1" to 4,
    ...
    @Test
    fun `enumerateCredentialsBegin returns NO_CREDENTIALS when RP has none`() = runTest {
    ...
    @Test
    fun `enumerateCredentialsGetNextCredential returns next credential after Begin`() = runTest {
        val cred1 = createTestCredential("cred1", "https://example.com", "user1")
        val cred2 = createTestCredential("cred2", "https://example.com", "user2")
        coEvery { credentialRepository.getCredentialsByRpId("https://example.com") } returns flowOf(cred1, cred2)
    ...
    @Test
    fun `enumerateCredentialsGetNextCredential returns NOT_ALLOWED when exhausted`() = runTest {
    ...
    private fun createTestCredential(id: String, rpId: String, userName: String): PasskeyCredential {

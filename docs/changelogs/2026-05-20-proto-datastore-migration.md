...
- **Cryptographic Modernization**: Mitigated compilation warnings in `AesSivEncryptionManager` (which has since been decommissioned and removed as part of the subsequent AES-SIV removal feature) by updating deprecated BouncyCastle `SICBlockCipher` and `AESEngine` constructors to their modern `newInstance()` factory equivalents.

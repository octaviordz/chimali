...
### User Story 7 — Single User Prompt per Authentication Ceremony (Priority: P1)

A user attempts to sign in via a relying party that triggers a FIDO2 authentication ceremony. The system presents exactly one confirmation prompt to the user, even if the underlying transport (e.g., Bluetooth HID) receives multiple identical `GetAssertion` requests due to host retries or timeouts.

**Why this priority**: Prompting the user multiple times for a single sign-in action severely degrades user experience, causes confusion, and may lead to authentication failures if the user cancels subsequent prompts.

**Independent Test**: Can be fully tested by simulating a host sending a `GetAssertion` request, followed immediately by a second identical request (simulating a timeout/retry), and verifying that only one UI prompt is displayed to the user.

**Acceptance Scenarios**:

1. **Given** an authentication ceremony is initiated and a `GetAssertion` request is received, **When** the host sends a duplicate `GetAssertion` request before the first one completes, **Then** the system presents only one confirmation prompt to the user.
2. **Given** a user approves the single prompt, **When** the system generates the assertion, **Then** the assertion is successfully returned for the pending request(s) without additional prompting.
...
- **FR-FIDO2-016**: The system MUST deduplicate incoming `GetAssertion` requests or maintain session state to ensure that the user is prompted only once per authentication ceremony, even if the host issues retries.
...
- **SC-FIDO2-009**: 100% of authentication ceremonies present exactly one user confirmation prompt, regardless of duplicate or overlapping `GetAssertion` requests from the host transport.

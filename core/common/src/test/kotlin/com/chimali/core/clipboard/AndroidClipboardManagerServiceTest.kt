...
class AndroidClipboardManagerServiceTest {
    ...
    @Test
    fun `copySensitiveData sets clip data and clears exactly after delay`() = runTest(testDispatcher) {
    ...
    @Test
    fun `clearClipboard explicitly clears clipboard and cancels pending job`() = runTest(testDispatcher) {
    ...
    @Test
    fun `successive copies reset the clear timer`() = runTest(testDispatcher) {
        every { clipboardManager.setPrimaryClip(any()) } just Runs
        every { clipboardManager.clearPrimaryClip() } just Runs
